...
UMAPServer <- function(id, setup_values, preproc_values) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$store_proc, {
      ...
      if (input$seg_choice == "anat_seg") {
        ...
        if (!is.null(x2$tf_list_anat) 
          ...
          if (!is.null(x2$data_list) && !is.null(x2$data_list$umap_separation$umap_out)) {
            umap_coords <- x2$data_list$umap_separation$umap_out
            tmp.img$UMAP1 <- umap_coords[x2$tf_list,1]
            tmp.img$UMAP2 <- umap_coords[x2$tf_list,2]
            tmp.img$UMAP3 <- umap_coords[x2$tf_list,3]
          }
    ...
    observeEvent(input$save_imzml, {
      ...
      # Reorder data with largest first
      pk_img_reorder <- x2$list_proc_img[order(sapply(x2$list_proc_img, ncol), decreasing = TRUE)]
      
      # Combine the reordered images
      pk_img <- combine_card(pk_img_reorder)
      
      # Add UMAP embeddings to pixel metadata if they exist
      if (!is.null(x2$data_list) && !is.null(x2$data_list$umap_separation$umap_out)) {
        # Create data frame of UMAP embeddings
        umap_df <- as.data.frame(x2$data_list$umap_separation$umap_out)
        colnames(umap_df) <- c("UMAP1", "UMAP2", "UMAP3")
        
        # Add embeddings to pixel metadata
        current_pdata <- as.data.frame(pData(pk_img))
        
        # Ensure the number of rows match
        if (nrow(umap_df) == nrow(current_pdata)) {
          # Combine existing pData with UMAP embeddings
          new_pdata <- cbind(current_pdata, umap_df)
          
          # Create new PositionDataFrame with UMAP data
          pData(pk_img) <- PositionDataFrame(
            coord = coord(pk_img),
            run = new_pdata$run,
            new_pdata[, !colnames(new_pdata) %in% c("run", "x", "y")]
          )
        } else {
          warning("UMAP embeddings dimensions don't match pixel data - embeddings not saved")
        }
      }
      
      # Clean up feature data
      fdata <- fData(pk_img)[unique(colnames(fData(pk_img)))]
      fData(pk_img) <- MassDataFrame(
        mz = fdata$mz, 
        fdata %>% as.data.frame() %>% dplyr::select(-mz)
      )
      
      # Save the file
      writeImzML(pk_img, filen)
      
      # Also save as RDS to preserve additional metadata
      saveRDS(pk_img, paste0(filen, ".rds"))
      
      showNotification(
        "Saved imzML file with UMAP embeddings in pixel metadata",
        type = "message"
      )
    })
