...
HeatmapServer <- function(id,  proc_values) {
  moduleServer(id, function(input, output, session) {
    ...
    ###outliers
    # based on stray package, described here:
    #https://arxiv.org/ftp/arxiv/papers/1908/1908.04000.pdf
    observeEvent(input$action_outliers, {
      #browser()
      #library(stray)
      dat <-
        tidyr::pivot_wider(x6$dat_long_tech_avg,
                           names_from = mz,
                           values_from = tech_avg)
      mz_only <- colnames(dat) %in% x6$dat_long_tech_avg$mz
      mat <- dat[, mz_only]
      pdat <- dat[,!mz_only]
      
      if (dim(pdat)[1] < input$hdo_k) {
        print("k larger than sample size, please check")
        return()
      }
      
      outliers <- stray::find_HDoutliers(
        mat,
        knnsearchtype = input$knnsearchtype,
        k = input$hdo_k,
        alpha = input$hdo_alpha,
        p = input$hdo_p,
        tn = input$hdo_p
      )
      print(outliers)
      x6$outliers <- outliers
      
      x6$outlier_labels <- pdat[outliers$outliers,]
      message("Outliers found:")
      print(as.data.frame(x6$outlier_labels))
    ...
    observeEvent(input$action_outliers_heatmap, {
      req(x6$dat_long_tech_avg)
      req(x6$outlier_labels)
      
      dat <- x6$dat_long_tech_avg
      
      dat_vec <- apply(dat, 1, paste0, collapse = "_")
      
      outlier_tf <-
        lapply(1:dim(x6$outlier_labels)[1], function(x)
          grepl(paste0(x6$outlier_labels[x,], collapse = "_"), dat_vec))
      
      
      
      #combine list of outliers, and get tf vector of all outliers
      outlier_final_tf <-
        (colSums(do.call("rbind", outlier_tf)) == 1)
      
      dat_no_outlier <- dat[!outlier_final_tf,]
      
      x6$dat_long_tech_avg <- dat_no_outlier
    })
    
    observeEvent(input$action_outliers_save, {
      req(proc_values()$x5$data_file_selected)
      req(x6$outlier_labels)
      
      pdat <-
        as.data.frame(pData(proc_values()$x5$data_file_selected))
      pdat_sel <- pdat[, colnames(x6$outlier_labels)]
      
      #create vector of selected columns from pdat for matching
      dat_vec <- apply(pdat_sel, 1, paste0, collapse = "_")
      
      outlier_final_tf <- NULL
      outlier_tf <- NULL
      outlier_tf <-
        lapply(1:dim(x6$outlier_labels)[1], function(x)
          grepl(paste0(x6$outlier_labels[x,], collapse = "_"), dat_vec))
      ...
      img.dat_outlier <-
        proc_values()$x5$data_file_selected[,!outlier_final_tf]
      
      saveRDS(
        img.dat_outlier,
        file = paste0(proc_values()$x5$source_file, "_OUTLIER_REMOVED.rds")
      )
      
      print(paste0(
        "File saved as ",
        paste0(proc_values()$x5$source_file, "_OUTLIER_REMOVED.rds")
      ))
