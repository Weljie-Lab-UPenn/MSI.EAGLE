...
input_path <- if (length(args) >= 1) args[1] else "/Users/aalim/Library/CloudStorage/Box-Box/DESI (Aalim Weljie)/Peak_list_sources/Positive_master_list_2025-02-01-TOH.txt"
...
clean_non_lipid_name <- function(x) {
  ...
  y <- gsub("(?i)^a-ketoglutarate$", "Alpha-ketoglutarate", y, perl = TRUE)
  y <- gsub("(?i)^cis-aconitate$", "cis-Aconitate", y, perl = TRUE)
  y <- gsub("(?i)^adenosine$", "Adenosine", y, perl = TRUE)
  y <- gsub("(?i)^unknown\\s*-\\s*", "Unknown: ", y, perl = TRUE)
  y <- gsub("\\s*\\((acrolein metab|free, non-esterified|or related isomer)\\)", "", y, ignore.case = TRUE)
  normalize_spaces(y)
}
...
formula_mz <- function(C, H, N = 0, O = 0, P = 0, adduct = proton_mass) {
  C * mono_mass[["C"]] +
    H * mono_mass[["H"]] +
    N * mono_mass[["N"]] +
    O * mono_mass[["O"]] +
...
theoretical_shorthand_mz <- function(label, carbon, db) {
  ...
  switch(
    label,
    LPC_H = formula_mz(8 + carbon, 18 + 2 * carbon - 2 * db, N = 1, O = 7, P = 1, adduct = proton_mass),
    LPC_O_H = formula_mz(8 + carbon, 20 + 2 * carbon - 2 * db, N = 1, O = 6, P = 1, adduct = proton_mass),
    LPC_O_NEUTRAL = formula_mz(8 + carbon, 20 + 2 * carbon - 2 * db, N = 1, O = 6, P = 1, adduct = 0),
    LPE_H = formula_mz(5 + carbon, 12 + 2 * carbon - 2 * db, N = 1, O = 7, P = 1, adduct = proton_mass),
    LPE_O_H = formula_mz(5 + carbon, 14 + 2 * carbon - 2 * db, N = 1, O = 6, P = 1, adduct = proton_mass),
    PC_H = formula_mz(8 + carbon, 16 + 2 * carbon - 2 * db, N = 1, O = 8, P = 1, adduct = proton_mass),
    PC_O_H = formula_mz(8 + carbon, 18 + 2 * carbon - 2 * db, N = 1, O = 7, P = 1, adduct = proton_mass),
    PE_O_H = formula_mz(5 + carbon, 12 + 2 * carbon - 2 * db, N = 1, O = 7, P = 1, adduct = proton_mass),
    DG_NH4 = formula_mz(3 + carbon, 4 + 2 * carbon - 2 * db, O = 5, adduct = nh4_mass),
    TG_NH4 = formula_mz(3 + carbon, 2 + 2 * carbon - 2 * db, O = 6, adduct = nh4_mass),
    CE_H = formula_mz(27 + carbon, 44 + 2 * carbon - 2 * db, O = 2, adduct = proton_mass),
    CE_NH4 = formula_mz(27 + carbon, 44 + 2 * carbon - 2 * db, O = 2, adduct = nh4_mass),
    SM_H = formula_mz(5 + carbon, 13 + 2 * carbon - 2 * db, N = 2, O = 6, P = 1, adduct = proton_mass),
    CER_SHORTHAND_H = formula_mz(carbon, 2 * carbon - 2 * db + 1, N = 1, O = 3, adduct = proton_mass),
    NA_real_
  )
}
...
resolve_shorthand_lipid_name <- function(class_text, carbon, mz, subtype = "") {
  ...
  if (identical(class_text, "LPC") && !nzchar(subtype)) {
    add_candidate("LPC_H", carbon)
    add_candidate("LPC_H", carbon - 1L)
    add_candidate("LPC_H", carbon + 1L)
  } else if (identical(class_text, "LPC") && identical(subtype, "O-")) {
    add_candidate("LPC_O_H", carbon)
    add_candidate("LPC_O_NEUTRAL", carbon)
    add_candidate("LPC_O_H", carbon - 1L)
    add_candidate("LPC_O_H", carbon + 1L)
  } else if (identical(class_text, "LPE") && !nzchar(subtype)) {
    add_candidate("LPE_H", carbon)
    add_candidate("LPE_H", carbon - 1L)
    add_candidate("LPE_H", carbon + 1L)
  } else if (identical(class_text, "LPE") && identical(subtype, "O-")) {
    add_candidate("LPE_O_H", carbon)
  } else if (identical(class_text, "PC") && !nzchar(subtype)) {
    add_candidate("PC_H", carbon)
    add_candidate("PC_H", carbon - 1L)
    add_candidate("PC_H", carbon + 1L)
  } else if (identical(class_text, "PC") && identical(subtype, "O-")) {
    add_candidate("PC_O_H", carbon)
    add_candidate("PC_O_H", carbon - 1L)
    add_candidate("PC_O_H", carbon + 1L)
  } else if (identical(class_text, "PE") && identical(subtype, "O-")) {
    add_candidate("PE_O_H", carbon)
  } else if (identical(class_text, "DG")) {
    add_candidate("DG_NH4", carbon)
  } else if (identical(class_text, "TG")) {
    add_candidate("TG_NH4", carbon)
  } else if (identical(class_text, "CE")) {
    add_candidate("CE_H", carbon)
    add_candidate("CE_NH4", carbon)
  } else if (identical(class_text, "SM")) {
    add_candidate("SM_H", carbon)
    add_candidate("SM_H", carbon + 18L)
  } else if (identical(class_text, "Cer")) {
    add_candidate("CER_SHORTHAND_H", carbon)
    add_candidate("CER_SHORTHAND_H", carbon + 2L)
  }
...
clean_acylcarnitine_name <- function(x) {
  y <- normalize_spaces(x)
  up <- toupper(y)

  if (grepl("CARNITINE", up) && !grepl("\\bCAR\\b|\\-CAR$", up)) {
    return("Carnitine")
  }

  if (!grepl("\\bCAR\\b|\\-CAR$", up)) {
    return(y)
  }

  base <- y
  base <- gsub("(?i)\\bCAR\\b", "", base, perl = TRUE)
  base <- gsub("(?i)-CAR$", "", base, perl = TRUE)
  base <- gsub("(?i)^C([0-9]{1,2}(?::[0-9]{1,2})?)\\s*", "C\\1 ", base, perl = TRUE)
  base <- gsub("^[[:space:]:;,_+\\-]+", "", base, perl = TRUE)
  base <- gsub("[[:space:]:;,_+\\-]+$", "", base, perl = TRUE)
  base <- gsub("\\s*-\\s*", "-", base, perl = TRUE)
  base <- gsub("\\s*\\+\\s*", "+", base, perl = TRUE)
  base <- gsub("C([0-9]{1,2})\\s*:\\s*([0-9]{1,2})", "C\\1:\\2", base, perl = TRUE)
  base <- gsub("C([0-9]{1,2}):(?=[\\-+])", "C\\1", base, perl = TRUE)
  ...
  sprintf("Acylcarnitine %s", base)
}

clean_coa_name <- function(x) {
  y <- normalize_spaces(x)
  if (!grepl("COA", toupper(y), fixed = TRUE)) return(y)

  y <- gsub("(?i)\\s*\\[M\\+H\\]\\+", "", y, perl = TRUE)
  y <- gsub("(?i)\\s*M\\+H(-2H2O|-H2O)?", "", y, perl = TRUE)
  y <- gsub("(?i)\\s*\\+H(-2H2O|-H2O)?", "", y, perl = TRUE)
  y <- gsub("(?i)\\bcoa\\b", "CoA", y, perl = TRUE)
  normalize_spaces(y)
}

clean_name_info <- function(x, mz = NA_real_) {
  first_pass <- clean_acylcarnitine_name(x)
  if (startsWith(first_pass, "Acylcarnitine") || identical(first_pass, "Carnitine")) {
    return(make_name_result(first_pass, 0L, ""))
  }

  coa <- clean_coa_name(first_pass)
  if (!identical(coa, first_pass)) {
    return(make_name_result(clean_non_lipid_name(coa), 0L, ""))
  }

  lipid <- clean_lipid_name_info(first_pass, mz = mz)
  if (!is.na(lipid$name)) {
    return(lipid)
  }

  make_name_result(clean_non_lipid_name(first_pass), 0L, "")
}
...
cardiolipin_2plus_table <- function() {
  # Explicit positive-mode CL targets with both [M+H]+ and [M+2H]2+.
  proton_mass <- 1.007276466812

  species <- data.frame(
    species = c(
      "CL(68:5)", "CL(68:4)", "CL(68:2)",
      "CL(70:5)", "CL(70:4)",
      "CL(72:8)", "CL(72:7)", "CL(72:6)", "CL(72:5)", "CL(72:4)",
      "CL(74:9)", "CL(74:8)", "CL(74:7)",
      "CL(76:12)", "CL(76:11)", "CL(76:10)", "CL(76:9)",
      "CL(78:12)", "CL(80:8)"
    ),
    mz_m2h2 = c(
      700.4856, 701.4934, 703.5090,
      714.5012, 715.5090,
      725.4934, 726.5012, 727.5090, 728.5169, 729.5247,
      738.5012, 739.5090, 740.5169,
      749.4934, 750.5012, 751.5090, 752.5169,
      763.5090, 781.5560
    ),
    mz_mh = c(
      NA, NA, NA,
      1427.9952, 1430.0108,
      1449.9795, 1451.9952, 1454.0108, 1456.0265, 1458.0421,
      1475.9952, 1478.0108, NA,
      1497.9795, 1499.9952, 1502.0108, 1504.0265,
      1526.0108, 1562.1047
    ),
    stringsAsFactors = FALSE
  )

  # Fill missing [M+H]+ using [M+2H]2+ relation: m/z_1+ = 2*m/z_2+ - H+
  fill_idx <- which(!is.finite(species$mz_mh) & is.finite(species$mz_m2h2))
  if (length(fill_idx) > 0) {
    species$mz_mh[fill_idx] <- 2 * species$mz_m2h2[fill_idx] - proton_mass
  }

  pos_1 <- data.frame(
    ID = sprintf("Cardiolipin %s [M+H]+ (putative)", species$species),
    exact.mass = species$mz_mh,
    name_rank = 0L,
    resolve_note = "",
    stringsAsFactors = FALSE
  )
  pos_2 <- data.frame(
    ID = sprintf("Cardiolipin %s [M+2H]2+ (putative)", species$species),
    exact.mass = species$mz_m2h2,
    name_rank = 0L,
    resolve_note = "",
    stringsAsFactors = FALSE
  )

  out <- rbind(pos_1, pos_2)
  out <- out[is.finite(out$exact.mass), c("ID", "exact.mass", "name_rank", "resolve_note"), drop = FALSE]
  out
}
...
collapse_global_ppm <- function(d, ppm = 5) {
  ...
  main <- audit[, c("ID", "exact.mass", "name_rank"), drop = FALSE]
...
# Append a curated CL block for positive-mode workflows.
cl_block <- cardiolipin_2plus_table()
...
base_name <- "Positive_master_list_2025-02-01-TOH_cleaned"
